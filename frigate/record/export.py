...
class RecordingExporter(threading.Thread):
    ...
    def __init__(
        ...
        self.resolved_quality: str | None = None
        ...
        if (self.end_time - self.start_time) <= MAX_PLAYLIST_SECONDS:
            ...
            for i in range(0, len(recordings), page_size):
                ...
                playlist_lines.append(
                    f"file 'http://127.0.0.1:{internal_port}/vod/{self.camera}/start/{float(chunk[0].start_time)}/end/{float(chunk[-1].end_time)}{vod_path_suffix}'"
                )
