...
class FrigateConfig(FrigateBaseModel):
    ...
    def post_validation(self, info: ValidationInfo) -> Self:
        ...
        for name, camera in self.cameras.items():
            ...
            # Note: the HD/SD live toggle is driven entirely by the UI. The UI
            # inspects `camera.ffmpeg.inputs` for a `record_events` role to
            # decide whether to show the toggle, and go2rtc's create_config.py
            # auto-registers a `<camera>_main` stream. No config mutation is
            # needed here.
